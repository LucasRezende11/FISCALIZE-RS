...
###################################################################################################################################
        ...
        for i, row in df_cidade.iterrows():
            fig.add_trace(go.Bar(x=[row['Ano']], y=[0.5], name=row['Ano'], text=f"<b>{row['Partido']}</b>",textposition='inside',hoverinfo="text", marker=dict(color='rgb(55, 83, 109)'),  customdata=[row['Candidato']],
